...
class BoostCoroutineRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'coroutine')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/coroutine/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_COROUTINE_NO_LIB=1']

        if len(targets) < 2:
            self.cpp_info.set_property('cmake_target_name', 'Boost::coroutine')
            self.cpp_info.set_property('b2_target_name', '/boost/coroutine//boost_coroutine')
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/coroutine//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/coroutine//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_coroutine : : : <include>include ;
...
alias boost_coroutine : libs/coroutine/build//boost_coroutine ;

install conan-install-headers
    : [ glob-tree-ex libs/coroutine/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/coroutine/include
    ;
alias conan-install : libs/coroutine/build//conan-install conan-install-headers ;
