...
class BoostCircularBufferRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'circular_buffer')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def build(self):
        ...
        b2.build('boost_circular_buffer')
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/circular_buffer/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_CIRCULAR_BUFFER_NO_LIB=1']
        ...
        if len(targets) < 2:
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/circular_buffer//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/circular_buffer//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_circular_buffer : : : <include>include ;
...
alias boost_circular_buffer : libs/circular_buffer/build//boost_circular_buffer ;

install conan-install-headers
    : [ glob-tree-ex libs/circular_buffer/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/circular_buffer/include
    ;
alias conan-install : libs/circular_buffer/build//conan-install conan-install-headers ;
