...
class BoostDescribeRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'describe')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/describe/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_DESCRIBE_NO_LIB=1']

        if len(targets) < 2:
            self.cpp_info.set_property('cmake_target_name', 'Boost::describe')
            self.cpp_info.set_property('b2_target_name', '/boost/describe//boost_describe')
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/describe//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/describe//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_describe : : : <include>include ;
...
alias boost_describe : libs/describe/build//boost_describe ;

install conan-install-headers
    : [ glob-tree-ex libs/describe/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/describe/include
    ;
alias conan-install : libs/describe/build//conan-install conan-install-headers ;
