...
class BoostUnitsRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'units')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/units/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_UNITS_NO_LIB=1']

        if len(targets) < 2:
            self.cpp_info.set_property('cmake_target_name', 'Boost::units')
            self.cpp_info.set_property('b2_target_name', '/boost/units//boost_units')
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/units//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/units//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_units : : : <include>include ;
...
alias boost_units : libs/units/build//boost_units ;

install conan-install-headers
    : [ glob-tree-ex libs/units/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/units/include
    ;
alias conan-install : libs/units/build//conan-install conan-install-headers ;
