...
class BoostStlInterfacesRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'stl_interfaces')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/stl_interfaces/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_STL_INTERFACES_NO_LIB=1']

        if len(targets) < 2:
            self.cpp_info.set_property('cmake_target_name', 'Boost::stl_interfaces')
            self.cpp_info.set_property('b2_target_name', '/boost/stl_interfaces//boost_stl_interfaces')
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/stl_interfaces//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/stl_interfaces//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_stl_interfaces : : : <include>include ;
...
alias boost_stl_interfaces : libs/stl_interfaces/build//boost_stl_interfaces ;

install conan-install-headers
    : [ glob-tree-ex libs/stl_interfaces/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/stl_interfaces/include
    ;
alias conan-install : libs/stl_interfaces/build//conan-install conan-install-headers ;
