...
class BoostParameterPythonRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'parameter_python')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def build(self):
        ...
        b2.build('boost_parameter_python')
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/parameter_python/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_PARAMETER_PYTHON_NO_LIB=1']

        if len(targets) < 2:
            self.cpp_info.set_property('cmake_target_name', 'Boost::parameter_python')
            self.cpp_info.set_property('b2_target_name', '/boost/parameter_python//boost_parameter_python')
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/parameter_python//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/parameter_python//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_parameter_python : : : <include>include ;
...
alias boost_parameter_python : libs/parameter_python/build//boost_parameter_python ;

install conan-install-headers
    : [ glob-tree-ex libs/parameter_python/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/parameter_python/include
    ;
alias conan-install : libs/parameter_python/build//conan-install conan-install-headers ;
