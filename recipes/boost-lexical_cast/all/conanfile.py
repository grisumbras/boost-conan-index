...
class BoostLexicalCastRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'lexical_cast')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/lexical_cast/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_LEXICAL_CAST_NO_LIB=1']
        ...
        if len(targets) < 2:
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/lexical_cast//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/lexical_cast//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_lexical_cast : : : <include>include ;
...
alias boost_lexical_cast : libs/lexical_cast/build//boost_lexical_cast ;

install conan-install-headers
    : [ glob-tree-ex libs/lexical_cast/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/lexical_cast/include
    ;
alias conan-install : libs/lexical_cast/build//conan-install conan-install-headers ;
