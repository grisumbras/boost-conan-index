...
class BoostFunctionRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'function')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/function/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_FUNCTION_NO_LIB=1']

        if len(targets) < 2:
            self.cpp_info.set_property('cmake_target_name', 'Boost::function')
            self.cpp_info.set_property('b2_target_name', '/boost/function//boost_function')
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/function//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/function//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_function : : : <include>include ;
...
alias boost_function : libs/function/build//boost_function ;

install conan-install-headers
    : [ glob-tree-ex libs/function/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/function/include
    ;
alias conan-install : libs/function/build//conan-install conan-install-headers ;
