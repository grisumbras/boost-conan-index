...
class BoostIntegerRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'integer')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/integer/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_INTEGER_NO_LIB=1']
        ...
        if len(targets) < 2:
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/integer//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/integer//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_integer : : : <include>include ;
...
alias boost_integer : libs/integer/build//boost_integer ;

install conan-install-headers
    : [ glob-tree-ex libs/integer/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/integer/include
    ;
alias conan-install : libs/integer/build//conan-install conan-install-headers ;
