...
class BoostPredefRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'predef')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/predef/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_PREDEF_NO_LIB=1']

        if len(targets) < 2:
            self.cpp_info.set_property('cmake_target_name', 'Boost::predef')
            self.cpp_info.set_property('b2_target_name', '/boost/predef//boost_predef')
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/predef//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/predef//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_predef : : : <include>include ;

call-if : boost-library predef ;
'''
...
alias boost_predef : libs/predef/build//boost_predef ;

install conan-install-headers
    : [ glob-tree-ex libs/predef/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/predef/include
    ;
alias conan-install : libs/predef/build//conan-install conan-install-headers ;
