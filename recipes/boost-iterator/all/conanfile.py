...
class BoostIteratorRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'iterator')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/iterator/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_ITERATOR_NO_LIB=1']

        if len(targets) < 2:
            self.cpp_info.set_property('cmake_target_name', 'Boost::iterator')
            self.cpp_info.set_property('b2_target_name', '/boost/iterator//boost_iterator')
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/iterator//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/iterator//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_iterator : : : <include>include ;
...
alias boost_iterator : libs/iterator/build//boost_iterator ;

install conan-install-headers
    : [ glob-tree-ex libs/iterator/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/iterator/include
    ;
alias conan-install : libs/iterator/build//conan-install conan-install-headers ;
