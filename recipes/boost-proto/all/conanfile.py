...
class BoostProtoRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'proto')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/proto/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_PROTO_NO_LIB=1']

        if len(targets) < 2:
            self.cpp_info.set_property('cmake_target_name', 'Boost::proto')
            self.cpp_info.set_property('b2_target_name', '/boost/proto//boost_proto')
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/proto//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/proto//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_proto : : : <include>include ;
...
alias boost_proto : libs/proto/build//boost_proto ;

install conan-install-headers
    : [ glob-tree-ex libs/proto/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/proto/include
    ;
alias conan-install : libs/proto/build//conan-install conan-install-headers ;
