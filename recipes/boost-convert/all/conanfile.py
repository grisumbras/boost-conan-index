...
class BoostConvertRecipe(ConanFile):
    name = 'boost-convert'
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'convert')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/convert/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_CONVERT_NO_LIB=1']

        if len(targets) < 2:
            self.cpp_info.set_property('cmake_target_name', 'Boost::convert')
            self.cpp_info.set_property('b2_target_name', '/boost/convert//boost_convert')
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/convert//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/convert//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_convert : : : <include>include ;
...
alias boost_convert : libs/convert/build//boost_convert ;

install conan-install-headers
    : [ glob-tree-ex libs/convert/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/convert/include
    ;
alias conan-install : libs/convert/build//conan-install conan-install-headers ;
