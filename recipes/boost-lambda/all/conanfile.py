...
class BoostLambdaRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'lambda')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/lambda/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_LAMBDA_NO_LIB=1']

        if len(targets) < 2:
            self.cpp_info.set_property('cmake_target_name', 'Boost::lambda')
            self.cpp_info.set_property('b2_target_name', '/boost/lambda//boost_lambda')
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/lambda//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/lambda//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_lambda : : : <include>include ;
...
alias boost_lambda : libs/lambda/build//boost_lambda ;

install conan-install-headers
    : [ glob-tree-ex libs/lambda/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/lambda/include
    ;
alias conan-install : libs/lambda/build//conan-install conan-install-headers ;
