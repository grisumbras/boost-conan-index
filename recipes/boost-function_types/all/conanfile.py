...
class BoostFunctionTypesRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'function_types')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        no_autolink = ['BOOST_FUNCTION_TYPES_NO_LIB=1']
        ...
        if len(targets) < 2:
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/function_types//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/function_types//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_function_types : : : <include>include ;
...
alias boost_function_types : libs/function_types/build//boost_function_types ;

install conan-install-headers
    : [ glob-tree-ex libs/function_types/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/function_types/include
    ;
alias conan-install : libs/function_types/build//conan-install conan-install-headers ;
