...
class BoostRegexRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'regex')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/regex/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_REGEX_NO_LIB=1']

        if len(targets) < 2:
            self.cpp_info.set_property('cmake_target_name', 'Boost::regex')
            self.cpp_info.set_property('b2_target_name', '/boost/regex//boost_regex')
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/regex//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/regex//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_regex : : : <include>include ;
...
alias boost_regex : libs/regex/build//boost_regex ;

install conan-install-headers
    : [ glob-tree-ex libs/regex/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/regex/include
    ;
alias conan-install : libs/regex/build//conan-install conan-install-headers ;
