...
class BoostTypeIndexRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'type_index')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/type_index/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_TYPE_INDEX_NO_LIB=1']

        if len(targets) < 2:
            self.cpp_info.set_property('cmake_target_name', 'Boost::type_index')
            self.cpp_info.set_property('b2_target_name', '/boost/type_index//boost_type_index')
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/type_index//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/type_index//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_type_index : : : <include>include ;
...
alias boost_type_index : libs/type_index/build//boost_type_index ;

install conan-install-headers
    : [ glob-tree-ex libs/type_index/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/type_index/include
    ;
alias conan-install : libs/type_index/build//conan-install conan-install-headers ;
