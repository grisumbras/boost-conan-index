...
class BoostEndianRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'endian')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/endian/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_ENDIAN_NO_LIB=1']

        if len(targets) < 2:
            self.cpp_info.set_property('cmake_target_name', 'Boost::endian')
            self.cpp_info.set_property('b2_target_name', '/boost/endian//boost_endian')
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/endian//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/endian//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_endian : : : <include>include ;
...
alias boost_endian : libs/endian/build//boost_endian ;

install conan-install-headers
    : [ glob-tree-ex libs/endian/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/endian/include
    ;
alias conan-install : libs/endian/build//conan-install conan-install-headers ;
