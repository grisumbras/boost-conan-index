...
class BoostCoroutine2Recipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'coroutine2')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/coroutine2/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_COROUTINE2_NO_LIB=1']
        ...
        if len(targets) < 2:
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/coroutine2//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/coroutine2//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_coroutine2 : : : <include>include ;
...
alias boost_coroutine2 : libs/coroutine2/build//boost_coroutine2 ;

install conan-install-headers
    : [ glob-tree-ex libs/coroutine2/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/coroutine2/include
    ;
alias conan-install : libs/coroutine2/build//conan-install conan-install-headers ;
