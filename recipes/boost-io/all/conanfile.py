...
class BoostIoRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'io')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/io/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_IO_NO_LIB=1']

        if len(targets) < 2:
            self.cpp_info.set_property('cmake_target_name', 'Boost::io')
            self.cpp_info.set_property('b2_target_name', '/boost/io//boost_io')
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/io//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/io//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_io : : : <include>include ;
...
alias boost_io : libs/io/build//boost_io ;

install conan-install-headers
    : [ glob-tree-ex libs/io/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/io/include
    ;
alias conan-install : libs/io/build//conan-install conan-install-headers ;
