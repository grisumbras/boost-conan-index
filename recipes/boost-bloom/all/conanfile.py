...
class BoostBloomRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'bloom')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/bloom/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_BLOOM_NO_LIB=1']

        if len(targets) < 2:
            self.cpp_info.set_property('cmake_target_name', 'Boost::bloom')
            self.cpp_info.set_property('b2_target_name', '/boost/bloom//boost_bloom')
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/bloom//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/bloom//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_bloom : : : <include>include ;
...
alias boost_bloom : libs/bloom/build//boost_bloom ;

install conan-install-headers
    : [ glob-tree-ex libs/bloom/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/bloom/include
    ;
alias conan-install : libs/bloom/build//conan-install conan-install-headers ;
