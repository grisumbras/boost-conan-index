...
class BoostBindRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'bind')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/bind/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_BIND_NO_LIB=1']

        if len(targets) < 2:
            self.cpp_info.set_property('cmake_target_name', 'Boost::bind')
            self.cpp_info.set_property('b2_target_name', '/boost/bind//boost_bind')
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/bind//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/bind//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_bind : : : <include>include ;
...
alias boost_bind : libs/bind/build//boost_bind ;

install conan-install-headers
    : [ glob-tree-ex libs/bind/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/bind/include
    ;
alias conan-install : libs/bind/build//conan-install conan-install-headers ;
