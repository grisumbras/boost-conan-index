...
class BoostDllRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'dll')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/dll/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_DLL_NO_LIB=1']

        if len(targets) < 2:
            self.cpp_info.set_property('cmake_target_name', 'Boost::dll')
            self.cpp_info.set_property('b2_target_name', '/boost/dll//boost_dll')
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/dll//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/dll//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_dll : : : <include>include ;
...
alias boost_dll : libs/dll/build//boost_dll ;

install conan-install-headers
    : [ glob-tree-ex libs/dll/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/dll/include
    ;
alias conan-install : libs/dll/build//conan-install conan-install-headers ;
