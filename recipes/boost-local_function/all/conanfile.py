...
class BoostLocalFunctionRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'local_function')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/local_function/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_LOCAL_FUNCTION_NO_LIB=1']
        ...
        if len(targets) < 2:
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/local_function//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/local_function//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_local_function : : : <include>include ;
...
alias boost_local_function : libs/local_function/build//boost_local_function ;

install conan-install-headers
    : [ glob-tree-ex libs/local_function/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/local_function/include
    ;
alias conan-install : libs/local_function/build//conan-install conan-install-headers ;
