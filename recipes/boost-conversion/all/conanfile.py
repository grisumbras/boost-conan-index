...
class BoostConversionRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'conversion')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/conversion/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_CONVERSION_NO_LIB=1']
        ...
        if len(targets) < 2:
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/conversion//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/conversion//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_conversion : : : <include>include ;
...
alias boost_conversion : libs/conversion/build//boost_conversion ;

install conan-install-headers
    : [ glob-tree-ex libs/conversion/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/conversion/include
    ;
alias conan-install : libs/conversion/build//conan-install conan-install-headers ;
