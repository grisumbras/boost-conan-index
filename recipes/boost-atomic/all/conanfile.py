...
class BoostAtomicRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'atomic')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/atomic/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_ATOMIC_NO_LIB=1']

        if len(targets) < 2:
            self.cpp_info.set_property('cmake_target_name', 'Boost::atomic')
            self.cpp_info.set_property('b2_target_name', '/boost/atomic//boost_atomic')
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/atomic//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/atomic//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_atomic : : : <include>include ;
...
alias boost_atomic : libs/atomic/build//boost_atomic ;

install conan-install-headers
    : [ glob-tree-ex libs/atomic/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/atomic/include
    ;
alias conan-install : libs/atomic/build//conan-install conan-install-headers ;
