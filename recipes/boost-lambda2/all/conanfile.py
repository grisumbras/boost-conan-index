...
class BoostLambda2Recipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'lambda2')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/lambda2/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_LAMBDA2_NO_LIB=1']

        if len(targets) < 2:
            self.cpp_info.set_property('cmake_target_name', 'Boost::lambda2')
            self.cpp_info.set_property('b2_target_name', '/boost/lambda2//boost_lambda2')
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/lambda2//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/lambda2//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_lambda2 : : : <include>include ;
...
alias boost_lambda2 : libs/lambda2/build//boost_lambda2 ;

install conan-install-headers
    : [ glob-tree-ex libs/lambda2/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/lambda2/include
    ;
alias conan-install : libs/lambda2/build//conan-install conan-install-headers ;
