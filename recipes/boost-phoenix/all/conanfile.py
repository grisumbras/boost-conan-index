...
class BoostPhoenixRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'phoenix')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/phoenix/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_PHOENIX_NO_LIB=1']

        if len(targets) < 2:
            self.cpp_info.set_property('cmake_target_name', 'Boost::phoenix')
            self.cpp_info.set_property('b2_target_name', '/boost/phoenix//boost_phoenix')
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/phoenix//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/phoenix//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_phoenix : : : <include>include ;
...
alias boost_phoenix : libs/phoenix/build//boost_phoenix ;

install conan-install-headers
    : [ glob-tree-ex libs/phoenix/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/phoenix/include
    ;
alias conan-install : libs/phoenix/build//conan-install conan-install-headers ;
