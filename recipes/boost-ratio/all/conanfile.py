...
class BoostRatioRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'ratio')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/ratio/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_RATIO_NO_LIB=1']

        if len(targets) < 2:
            self.cpp_info.set_property('cmake_target_name', 'Boost::ratio')
            self.cpp_info.set_property('b2_target_name', '/boost/ratio//boost_ratio')
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/ratio//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/ratio//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_ratio : : : <include>include ;
...
alias boost_ratio : libs/ratio/build//boost_ratio ;

install conan-install-headers
    : [ glob-tree-ex libs/ratio/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/ratio/include
    ;
alias conan-install : libs/ratio/build//conan-install conan-install-headers ;
