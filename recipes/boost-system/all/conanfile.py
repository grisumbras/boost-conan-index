...
class BoostSystemRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'system')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/system/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_SYSTEM_NO_LIB=1']

        if len(targets) < 2:
            self.cpp_info.set_property('cmake_target_name', 'Boost::system')
            self.cpp_info.set_property('b2_target_name', '/boost/system//boost_system')
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/system//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/system//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_system : : : <include>include ;
...
alias boost_system : libs/system/build//boost_system ;

install conan-install-headers
    : [ glob-tree-ex libs/system/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/system/include
    ;
alias conan-install : libs/system/build//conan-install conan-install-headers ;
