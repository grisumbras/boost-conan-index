...
class BoostRandomRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'random')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/random/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_RANDOM_NO_LIB=1']

        if len(targets) < 2:
            self.cpp_info.set_property('cmake_target_name', 'Boost::random')
            self.cpp_info.set_property('b2_target_name', '/boost/random//boost_random')
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/random//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/random//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_random : : : <include>include ;
...
alias boost_random : libs/random/build//boost_random ;

install conan-install-headers
    : [ glob-tree-ex libs/random/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/random/include
    ;
alias conan-install : libs/random/build//conan-install conan-install-headers ;
