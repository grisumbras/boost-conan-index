...
class BoostCompatRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'compat')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/compat/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_COMPAT_NO_LIB=1']

        if len(targets) < 2:
            self.cpp_info.set_property('cmake_target_name', 'Boost::compat')
            self.cpp_info.set_property('b2_target_name', '/boost/compat//boost_compat')
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/compat//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/compat//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_compat : : : <include>include ;
...
alias boost_compat : libs/compat/build//boost_compat ;

install conan-install-headers
    : [ glob-tree-ex libs/compat/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/compat/include
    ;
alias conan-install : libs/compat/build//conan-install conan-install-headers ;
