...
class BoostMultiprecisionRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'multiprecision')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/multiprecision/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_MULTIPRECISION_NO_LIB=1']

        if len(targets) < 2:
            self.cpp_info.set_property('cmake_target_name', 'Boost::multiprecision')
            self.cpp_info.set_property('b2_target_name', '/boost/multiprecision//boost_multiprecision')
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/multiprecision//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/multiprecision//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_multiprecision : : : <include>include ;
...
alias boost_multiprecision : libs/multiprecision/build//boost_multiprecision ;

install conan-install-headers
    : [ glob-tree-ex libs/multiprecision/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/multiprecision/include
    ;
alias conan-install : libs/multiprecision/build//conan-install conan-install-headers ;
