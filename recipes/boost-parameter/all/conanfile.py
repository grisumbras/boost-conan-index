...
class BoostParameterRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'parameter')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/parameter/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_PARAMETER_NO_LIB=1']

        if len(targets) < 2:
            self.cpp_info.set_property('cmake_target_name', 'Boost::parameter')
            self.cpp_info.set_property('b2_target_name', '/boost/parameter//boost_parameter')
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/parameter//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/parameter//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_parameter : : : <include>include ;
...
alias boost_parameter : libs/parameter/build//boost_parameter ;

install conan-install-headers
    : [ glob-tree-ex libs/parameter/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/parameter/include
    ;
alias conan-install : libs/parameter/build//conan-install conan-install-headers ;
