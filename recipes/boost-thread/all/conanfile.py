...
class BoostThreadRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'thread')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/thread/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_THREAD_NO_LIB=1']

        if len(targets) < 2:
            self.cpp_info.set_property('cmake_target_name', 'Boost::thread')
            self.cpp_info.set_property('b2_target_name', '/boost/thread//boost_thread')
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/thread//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/thread//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_thread : : : <include>include ;
...
alias boost_thread : libs/thread/build//boost_thread ;

install conan-install-headers
    : [ glob-tree-ex libs/thread/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/thread/include
    ;
alias conan-install : libs/thread/build//conan-install conan-install-headers ;
