...
class BoostDateTimeRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'date_time')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/date_time/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_DATE_TIME_NO_LIB=1']
        ...
        if len(targets) < 2:
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/date_time//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/date_time//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_date_time : : : <include>include ;
...
alias boost_date_time : libs/date_time/build//boost_date_time ;

install conan-install-headers
    : [ glob-tree-ex libs/date_time/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/date_time/include
    ;
alias conan-install : libs/date_time/build//conan-install conan-install-headers ;
