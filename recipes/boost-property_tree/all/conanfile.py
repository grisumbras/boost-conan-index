...
class BoostPropertyTreeRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'property_tree')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/property_tree/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_PROPERTY_TREE_NO_LIB=1']

        if len(targets) < 2:
            self.cpp_info.set_property('cmake_target_name', 'Boost::property_tree')
            self.cpp_info.set_property('b2_target_name', '/boost/property_tree//boost_property_tree')
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/property_tree//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/property_tree//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_property_tree : : : <include>include ;
...
alias boost_property_tree : libs/property_tree/build//boost_property_tree ;

install conan-install-headers
    : [ glob-tree-ex libs/property_tree/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/property_tree/include
    ;
alias conan-install : libs/property_tree/build//conan-install conan-install-headers ;
