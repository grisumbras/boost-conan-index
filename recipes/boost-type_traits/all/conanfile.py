...
class BoostTypeTraitsRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'type_traits')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/type_traits/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_TYPE_TRAITS_NO_LIB=1']
        ...
        if len(targets) < 2:
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/type_traits//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/type_traits//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_type_traits : : : <include>include ;
...
alias boost_type_traits : libs/type_traits/build//boost_type_traits ;

install conan-install-headers
    : [ glob-tree-ex libs/type_traits/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/type_traits/include
    ;
alias conan-install : libs/type_traits/build//conan-install conan-install-headers ;
