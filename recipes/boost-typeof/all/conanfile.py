...
class BoostTypeofRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'typeof')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/typeof/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_TYPEOF_NO_LIB=1']

        if len(targets) < 2:
            self.cpp_info.set_property('cmake_target_name', 'Boost::typeof')
            self.cpp_info.set_property('b2_target_name', '/boost/typeof//boost_typeof')
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/typeof//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/typeof//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_typeof : : : <include>include ;
...
alias boost_typeof : libs/typeof/build//boost_typeof ;

install conan-install-headers
    : [ glob-tree-ex libs/typeof/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/typeof/include
    ;
alias conan-install : libs/typeof/build//conan-install conan-install-headers ;
