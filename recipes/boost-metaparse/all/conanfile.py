...
class BoostMetaparseRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'metaparse')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/metaparse/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_METAPARSE_NO_LIB=1']

        if len(targets) < 2:
            self.cpp_info.set_property('cmake_target_name', 'Boost::metaparse')
            self.cpp_info.set_property('b2_target_name', '/boost/metaparse//boost_metaparse')
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/metaparse//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/metaparse//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_metaparse : : : <include>include ;
...
alias boost_metaparse : libs/metaparse/build//boost_metaparse ;

install conan-install-headers
    : [ glob-tree-ex libs/metaparse/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/metaparse/include
    ;
alias conan-install : libs/metaparse/build//conan-install conan-install-headers ;
