...
class BoostMoveRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'move')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/move/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_MOVE_NO_LIB=1']

        if len(targets) < 2:
            self.cpp_info.set_property('cmake_target_name', 'Boost::move')
            self.cpp_info.set_property('b2_target_name', '/boost/move//boost_move')
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/move//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/move//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_move : : : <include>include ;
...
alias boost_move : libs/move/build//boost_move ;

install conan-install-headers
    : [ glob-tree-ex libs/move/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/move/include
    ;
alias conan-install : libs/move/build//conan-install conan-install-headers ;
