...
class BoostArrayRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'array')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/array/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_ARRAY_NO_LIB=1']

        if len(targets) < 2:
            self.cpp_info.set_property('cmake_target_name', 'Boost::array')
            self.cpp_info.set_property('b2_target_name', '/boost/array//boost_array')
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/array//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/array//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_array : : : <include>include ;
...
alias boost_array : libs/array/build//boost_array ;

install conan-install-headers
    : [ glob-tree-ex libs/array/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/array/include
    ;
alias conan-install : libs/array/build//conan-install conan-install-headers ;
