...
class BoostSerializationRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'serialization')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/serialization/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_SERIALIZATION_NO_LIB=1']

        if len(targets) < 2:
            self.cpp_info.set_property('cmake_target_name', 'Boost::serialization')
            self.cpp_info.set_property('b2_target_name', '/boost/serialization//boost_serialization')
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/serialization//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/serialization//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_serialization : : : <include>include ;
...
alias boost_serialization : libs/serialization/build//boost_serialization ;

install conan-install-headers
    : [ glob-tree-ex libs/serialization/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/serialization/include
    ;
alias conan-install : libs/serialization/build//conan-install conan-install-headers ;
