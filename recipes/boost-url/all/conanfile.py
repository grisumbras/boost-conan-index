...
class BoostUrlRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'url')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/url/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_URL_NO_LIB=1']

        if len(targets) < 2:
            self.cpp_info.set_property('cmake_target_name', 'Boost::url')
            self.cpp_info.set_property('b2_target_name', '/boost/url//boost_url')
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/url//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/url//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_url : : : <include>include ;
...
alias boost_url : libs/url/build//boost_url ;

install conan-install-headers
    : [ glob-tree-ex libs/url/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/url/include
    ;
alias conan-install : libs/url/build//conan-install conan-install-headers ;
