...
class BoostNumericConversionRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'numeric_conversion')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def build(self):
        ...
        b2.build('boost_numeric_conversion')
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/numeric_conversion/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_NUMERIC_CONVERSION_NO_LIB=1']

        if len(targets) < 2:
            self.cpp_info.set_property('cmake_target_name', 'Boost::numeric_conversion')
            self.cpp_info.set_property('b2_target_name', '/boost/numeric_conversion//boost_numeric_conversion')
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/numeric_conversion//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/numeric_conversion//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_numeric_conversion : : : <include>include ;
...
alias boost_numeric_conversion : libs/numeric_conversion/build//boost_numeric_conversion ;

install conan-install-headers
    : [ glob-tree-ex libs/numeric_conversion/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/numeric_conversion/include
    ;
alias conan-install : libs/numeric_conversion/build//conan-install conan-install-headers ;
