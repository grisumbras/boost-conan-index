...
class BoostLogicRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'logic')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/logic/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_LOGIC_NO_LIB=1']

        if len(targets) < 2:
            self.cpp_info.set_property('cmake_target_name', 'Boost::logic')
            self.cpp_info.set_property('b2_target_name', '/boost/logic//boost_logic')
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/logic//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/logic//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_logic : : : <include>include ;
...
alias boost_logic : libs/logic/build//boost_logic ;

install conan-install-headers
    : [ glob-tree-ex libs/logic/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/logic/include
    ;
alias conan-install : libs/logic/build//conan-install conan-install-headers ;
