...
class BoostMultiIndexRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'multi_index')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/multi_index/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_MULTI_INDEX_NO_LIB=1']

        if len(targets) < 2:
            self.cpp_info.set_property('cmake_target_name', 'Boost::multi_index')
            self.cpp_info.set_property('b2_target_name', '/boost/multi_index//boost_multi_index')
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/multi_index//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/multi_index//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_multi_index : : : <include>include ;
...
alias boost_multi_index : libs/multi_index/build//boost_multi_index ;

install conan-install-headers
    : [ glob-tree-ex libs/multi_index/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/multi_index/include
    ;
alias conan-install : libs/multi_index/build//conan-install conan-install-headers ;
