...
class BoostFunctionalRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'functional')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/functional/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_FUNCTIONAL_NO_LIB=1']

        if len(targets) < 2:
            self.cpp_info.set_property('cmake_target_name', 'Boost::functional')
            self.cpp_info.set_property('b2_target_name', '/boost/functional//boost_functional')
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/functional//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/functional//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_functional : : : <include>include ;
...
alias boost_functional : libs/functional/build//boost_functional ;

install conan-install-headers
    : [ glob-tree-ex libs/functional/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/functional/include
    ;
alias conan-install : libs/functional/build//conan-install conan-install-headers ;
