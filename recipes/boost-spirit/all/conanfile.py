    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'spirit')
                mkdir(self, lib_dir)
                for file in files:
...
alias boost_spirit : : : <include>include ;
...
alias boost_spirit : libs/spirit/build//boost_spirit ;

install conan-install-headers
    : [ glob-tree-ex libs/spirit/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/spirit/include
    ;
alias conan-install : libs/spirit/build//conan-install conan-install-headers ;
