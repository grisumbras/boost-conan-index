...
class BoostHofRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'hof')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/hof/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_HOF_NO_LIB=1']

        if len(targets) < 2:
            self.cpp_info.set_property('cmake_target_name', 'Boost::hof')
            self.cpp_info.set_property('b2_target_name', '/boost/hof//boost_hof')
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/hof//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/hof//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_hof : : : <include>include ;
...
alias boost_hof : libs/hof/build//boost_hof ;

install conan-install-headers
    : [ glob-tree-ex libs/hof/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/hof/include
    ;
alias conan-install : libs/hof/build//conan-install conan-install-headers ;
