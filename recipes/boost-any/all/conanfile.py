...
class BoostAnyRecipe(ConanFile):
    name = 'boost-any'
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'any')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/any/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_ANY_NO_LIB=1']

        if len(targets) < 2:
            self.cpp_info.set_property('cmake_target_name', 'Boost::any')
            self.cpp_info.set_property('b2_target_name', '/boost/any//boost_any')
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/any//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/any//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_any : : : <include>include ;
...
alias boost_any : libs/any/build//boost_any ;

install conan-install-headers
    : [ glob-tree-ex libs/any/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/any/include
    ;
alias conan-install : libs/any/build//conan-install conan-install-headers ;
