...
class BoostScopeExitRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'scope_exit')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/scope_exit/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_SCOPE_EXIT_NO_LIB=1']
        ...
        if len(targets) < 2:
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/scope_exit//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/scope_exit//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_scope_exit : : : <include>include ;
...
alias boost_scope_exit : libs/scope_exit/build//boost_scope_exit ;

install conan-install-headers
    : [ glob-tree-ex libs/scope_exit/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/scope_exit/include
    ;
alias conan-install : libs/scope_exit/build//conan-install conan-install-headers ;
