...
class BoostGilRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'gil')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/gil/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_GIL_NO_LIB=1']
        ...
        if len(targets) < 2:
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/gil//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/gil//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_gil : : : <include>include ;
...
alias boost_gil : libs/gil/build//boost_gil ;

install conan-install-headers
    : [ glob-tree-ex libs/gil/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/gil/include
    ;
alias conan-install : libs/gil/build//conan-install conan-install-headers ;
