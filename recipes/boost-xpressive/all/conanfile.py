...
class BoostXpressiveRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'xpressive')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/xpressive/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_XPRESSIVE_NO_LIB=1']

        if len(targets) < 2:
            self.cpp_info.set_property('cmake_target_name', 'Boost::xpressive')
            self.cpp_info.set_property('b2_target_name', '/boost/xpressive//boost_xpressive')
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/xpressive//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/xpressive//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_xpressive : : : <include>include ;
...
alias boost_xpressive : libs/xpressive/build//boost_xpressive ;

install conan-install-headers
    : [ glob-tree-ex libs/xpressive/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/xpressive/include
    ;
alias conan-install : libs/xpressive/build//conan-install conan-install-headers ;
