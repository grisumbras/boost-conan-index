...
class BoostMqtt5Recipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'mqtt5')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/mqtt5/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_MQTT5_NO_LIB=1']

        if len(targets) < 2:
            self.cpp_info.set_property('cmake_target_name', 'Boost::mqtt5')
            self.cpp_info.set_property('b2_target_name', '/boost/mqtt5//boost_mqtt5')
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/mqtt5//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/mqtt5//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_mqtt5 : : : <include>include ;
...
alias boost_mqtt5 : libs/mqtt5/build//boost_mqtt5 ;

install conan-install-headers
    : [ glob-tree-ex libs/mqtt5/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/mqtt5/include
    ;
alias conan-install : libs/mqtt5/build//conan-install conan-install-headers ;
