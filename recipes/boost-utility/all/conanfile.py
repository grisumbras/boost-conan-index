...
class BoostUtilityRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'utility')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/utility/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_UTILITY_NO_LIB=1']

        if len(targets) < 2:
            self.cpp_info.set_property('cmake_target_name', 'Boost::utility')
            self.cpp_info.set_property('b2_target_name', '/boost/utility//boost_utility')
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/utility//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/utility//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_utility : : : <include>include ;
...
alias boost_utility : libs/utility/build//boost_utility ;

install conan-install-headers
    : [ glob-tree-ex libs/utility/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/utility/include
    ;
alias conan-install : libs/utility/build//conan-install conan-install-headers ;
