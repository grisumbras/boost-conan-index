...
class BoostPythonRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'python')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/python/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_PYTHON_NO_LIB=1']

        if len(targets) < 2:
            self.cpp_info.set_property('cmake_target_name', 'Boost::python')
            self.cpp_info.set_property('b2_target_name', '/boost/python//boost_python')
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/python//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/python//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_python : : : <include>include ;
...
alias boost_python : libs/python/build//boost_python ;

install conan-install-headers
    : [ glob-tree-ex libs/python/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/python/include
    ;
alias conan-install : libs/python/build//conan-install conan-install-headers ;
