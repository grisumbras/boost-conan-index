...
class BoostSafeNumericsRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'safe_numerics')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/safe_numerics/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_SAFE_NUMERICS_NO_LIB=1']

        if len(targets) < 2:
            self.cpp_info.set_property('cmake_target_name', 'Boost::safe_numerics')
            self.cpp_info.set_property('b2_target_name', '/boost/safe_numerics//boost_safe_numerics')
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/safe_numerics//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/safe_numerics//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_safe_numerics : : : <include>include ;
...
alias boost_safe_numerics : libs/safe_numerics/build//boost_safe_numerics ;

install conan-install-headers
    : [ glob-tree-ex libs/safe_numerics/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/safe_numerics/include
    ;
alias conan-install : libs/safe_numerics/build//conan-install conan-install-headers ;
