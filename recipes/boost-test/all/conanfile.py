...
class BoostTestRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'test')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/test/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_TEST_NO_LIB=1']

        if len(targets) < 2:
            self.cpp_info.set_property('cmake_target_name', 'Boost::test')
            self.cpp_info.set_property('b2_target_name', '/boost/test//boost_test')
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/test//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/test//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_test : : : <include>include ;
...
alias boost_test : libs/test/build//boost_test ;

install conan-install-headers
    : [ glob-tree-ex libs/test/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/test/include
    ;
alias conan-install : libs/test/build//conan-install conan-install-headers ;
