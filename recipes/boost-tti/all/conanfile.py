...
class BoostTtiRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'tti')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/tti/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_TTI_NO_LIB=1']

        if len(targets) < 2:
            self.cpp_info.set_property('cmake_target_name', 'Boost::tti')
            self.cpp_info.set_property('b2_target_name', '/boost/tti//boost_tti')
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/tti//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/tti//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_tti : : : <include>include ;
...
alias boost_tti : libs/tti/build//boost_tti ;

install conan-install-headers
    : [ glob-tree-ex libs/tti/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/tti/include
    ;
alias conan-install : libs/tti/build//conan-install conan-install-headers ;
