...
class BoostPtrContainerRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'ptr_container')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/ptr_container/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_PTR_CONTAINER_NO_LIB=1']

        if len(targets) < 2:
            self.cpp_info.set_property('cmake_target_name', 'Boost::ptr_container')
            self.cpp_info.set_property('b2_target_name', '/boost/ptr_container//boost_ptr_container')
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/ptr_container//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/ptr_container//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_ptr_container : : : <include>include ;
...
alias boost_ptr_container : libs/ptr_container/build//boost_ptr_container ;

install conan-install-headers
    : [ glob-tree-ex libs/ptr_container/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/ptr_container/include
    ;
alias conan-install : libs/ptr_container/build//conan-install conan-install-headers ;
