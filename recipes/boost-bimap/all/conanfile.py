...
class BoostBimapRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'bimap')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/bimap/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_BIMAP_NO_LIB=1']

        if len(targets) < 2:
            self.cpp_info.set_property('cmake_target_name', 'Boost::bimap')
            self.cpp_info.set_property('b2_target_name', '/boost/bimap//boost_bimap')
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/bimap//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/bimap//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_bimap : : : <include>include ;
...
alias boost_bimap : libs/bimap/build//boost_bimap ;

install conan-install-headers
    : [ glob-tree-ex libs/bimap/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/bimap/include
    ;
alias conan-install : libs/bimap/build//conan-install conan-install-headers ;
