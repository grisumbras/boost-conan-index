...
class BoostProgramOptionsRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'program_options')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def build(self):
        ...
        b2.build('boost_program_options')
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/program_options/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_PROGRAM_OPTIONS_NO_LIB=1']

        if len(targets) < 2:
            self.cpp_info.set_property('cmake_target_name', 'Boost::program_options')
            self.cpp_info.set_property('b2_target_name', '/boost/program_options//boost_program_options')
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/program_options//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/program_options//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_program_options : : : <include>include ;
...
alias boost_program_options : libs/program_options/build//boost_program_options ;

install conan-install-headers
    : [ glob-tree-ex libs/program_options/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/program_options/include
    ;
alias conan-install : libs/program_options/build//conan-install conan-install-headers ;
