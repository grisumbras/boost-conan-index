...
class BoostUblasRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'ublas')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/ublas/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_UBLAS_NO_LIB=1']
        ...
        if len(targets) < 2:
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/ublas//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/ublas//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_ublas : : : <include>include ;
...
alias boost_ublas : libs/ublas/build//boost_ublas ;

install conan-install-headers
    : [ glob-tree-ex libs/ublas/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/ublas/include
    ;
alias conan-install : libs/ublas/build//conan-install conan-install-headers ;
