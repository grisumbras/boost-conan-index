...
class BoostFormatRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'format')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/format/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_FORMAT_NO_LIB=1']

        if len(targets) < 2:
            self.cpp_info.set_property('cmake_target_name', 'Boost::format')
            self.cpp_info.set_property('b2_target_name', '/boost/format//boost_format')
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/format//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/format//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_format : : : <include>include ;
...
alias boost_format : libs/format/build//boost_format ;

install conan-install-headers
    : [ glob-tree-ex libs/format/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/format/include
    ;
alias conan-install : libs/format/build//conan-install conan-install-headers ;
