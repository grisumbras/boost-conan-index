...
class BoostSmartPtrRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'smart_ptr')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/smart_ptr/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_SMART_PTR_NO_LIB=1']
        ...
        if len(targets) < 2:
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/smart_ptr//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/smart_ptr//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_smart_ptr : : : <include>include ;
...
alias boost_smart_ptr : libs/smart_ptr/build//boost_smart_ptr ;

install conan-install-headers
    : [ glob-tree-ex libs/smart_ptr/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/smart_ptr/include
    ;
alias conan-install : libs/smart_ptr/build//conan-install conan-install-headers ;
