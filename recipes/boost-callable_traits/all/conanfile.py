...
class BoostCallableTraitsRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'callable_traits')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def build(self):
        ...
        b2.build('boost_callable_traits')
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/callable_traits/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_CALLABLE_TRAITS_NO_LIB=1']

        if len(targets) < 2:
            self.cpp_info.set_property('cmake_target_name', 'Boost::callable_traits')
            self.cpp_info.set_property('b2_target_name', '/boost/callable_traits//boost_callable_traits')
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/callable_traits//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/callable_traits//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_callable_traits : : : <include>include ;
...
alias boost_callable_traits : libs/callable_traits/build//boost_callable_traits ;

install conan-install-headers
    : [ glob-tree-ex libs/callable_traits/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/callable_traits/include
    ;
alias conan-install : libs/callable_traits/build//conan-install conan-install-headers ;
