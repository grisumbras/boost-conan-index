...
class BoostWinapiRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'winapi')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/winapi/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_WINAPI_NO_LIB=1']

        if len(targets) < 2:
            self.cpp_info.set_property('cmake_target_name', 'Boost::winapi')
            self.cpp_info.set_property('b2_target_name', '/boost/winapi//boost_winapi')
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/winapi//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/winapi//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_winapi : : : <include>include ;
...
alias boost_winapi : libs/winapi/build//boost_winapi ;

install conan-install-headers
    : [ glob-tree-ex libs/winapi/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/winapi/include
    ;
alias conan-install : libs/winapi/build//conan-install conan-install-headers ;
