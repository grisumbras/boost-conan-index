...
class BoostCrcRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'crc')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/crc/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_CRC_NO_LIB=1']

        if len(targets) < 2:
            self.cpp_info.set_property('cmake_target_name', 'Boost::crc')
            self.cpp_info.set_property('b2_target_name', '/boost/crc//boost_crc')
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/crc//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/crc//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_crc : : : <include>include ;
...
alias boost_crc : libs/crc/build//boost_crc ;

install conan-install-headers
    : [ glob-tree-ex libs/crc/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/crc/include
    ;
alias conan-install : libs/crc/build//conan-install conan-install-headers ;
