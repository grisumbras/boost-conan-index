...
class BoostContainerHashRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'container_hash')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/container_hash/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_CONTAINER_HASH_NO_LIB=1']

        if len(targets) < 2:
            self.cpp_info.set_property('cmake_target_name', 'Boost::container_hash')
            self.cpp_info.set_property('b2_target_name', '/boost/container_hash//boost_container_hash')
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/container_hash//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/container_hash//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_container_hash : : : <include>include ;
...
alias boost_container_hash : libs/container_hash/build//boost_container_hash ;

install conan-install-headers
    : [ glob-tree-ex libs/container_hash/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/container_hash/include
    ;
alias conan-install : libs/container_hash/build//conan-install conan-install-headers ;
