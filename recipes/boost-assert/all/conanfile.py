...
class BoostAssertRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'assert')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/assert/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_ASSERT_NO_LIB=1']

        if len(targets) < 2:
            self.cpp_info.set_property('cmake_target_name', 'Boost::assert')
            self.cpp_info.set_property('b2_target_name', '/boost/assert//boost_assert')
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/assert//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/assert//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_assert : : : <include>include ;
...
alias boost_assert : libs/assert/build//boost_assert ;

install conan-install-headers
    : [ glob-tree-ex libs/assert/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/assert/include
    ;
alias conan-install : libs/assert/build//conan-install conan-install-headers ;
