...
class BoostBeastRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'beast')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/beast/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_BEAST_NO_LIB=1']

        if len(targets) < 2:
            self.cpp_info.set_property('cmake_target_name', 'Boost::beast')
            self.cpp_info.set_property('b2_target_name', '/boost/beast//boost_beast')
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/beast//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/beast//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_beast : : : <include>include ;
...
alias boost_beast : libs/beast/build//boost_beast ;

install conan-install-headers
    : [ glob-tree-ex libs/beast/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/beast/include
    ;
alias conan-install : libs/beast/build//conan-install conan-install-headers ;
