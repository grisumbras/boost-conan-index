...
class BoostContractRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'contract')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/contract/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_CONTRACT_NO_LIB=1']

        if len(targets) < 2:
            self.cpp_info.set_property('cmake_target_name', 'Boost::contract')
            self.cpp_info.set_property('b2_target_name', '/boost/contract//boost_contract')
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/contract//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/contract//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_contract : : : <include>include ;
...
alias boost_contract : libs/contract/build//boost_contract ;

install conan-install-headers
    : [ glob-tree-ex libs/contract/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/contract/include
    ;
alias conan-install : libs/contract/build//conan-install conan-install-headers ;
