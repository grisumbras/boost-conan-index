...
class BoostCoreRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'core')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/core/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_CORE_NO_LIB=1']
        ...
        if len(targets) < 2:
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/core//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/core//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_core : : : <include>include ;
...
alias boost_core : libs/core/build//boost_core ;

install conan-install-headers
    : [ glob-tree-ex libs/core/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/core/include
    ;
alias conan-install : libs/core/build//conan-install conan-install-headers ;
