...
class BoostOpenmethodRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'openmethod')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/openmethod/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_OPENMETHOD_NO_LIB=1']

        if len(targets) < 2:
            self.cpp_info.set_property('cmake_target_name', 'Boost::openmethod')
            self.cpp_info.set_property('b2_target_name', '/boost/openmethod//boost_openmethod')
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/openmethod//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/openmethod//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_openmethod : : : <include>include ;
...
alias boost_openmethod : libs/openmethod/build//boost_openmethod ;

install conan-install-headers
    : [ glob-tree-ex libs/openmethod/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/openmethod/include
    ;
alias conan-install : libs/openmethod/build//conan-install conan-install-headers ;
