...
class BoostYapRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'yap')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/yap/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_YAP_NO_LIB=1']

        if len(targets) < 2:
            self.cpp_info.set_property('cmake_target_name', 'Boost::yap')
            self.cpp_info.set_property('b2_target_name', '/boost/yap//boost_yap')
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/yap//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/yap//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_yap : : : <include>include ;
...
alias boost_yap : libs/yap/build//boost_yap ;

install conan-install-headers
    : [ glob-tree-ex libs/yap/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/yap/include
    ;
alias conan-install : libs/yap/build//conan-install conan-install-headers ;
