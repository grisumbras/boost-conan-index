...
class BoostInterprocessRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'interprocess')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/interprocess/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_INTERPROCESS_NO_LIB=1']

        if len(targets) < 2:
            self.cpp_info.set_property('cmake_target_name', 'Boost::interprocess')
            self.cpp_info.set_property('b2_target_name', '/boost/interprocess//boost_interprocess')
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/interprocess//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/interprocess//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_interprocess : : : <include>include ;
...
alias boost_interprocess : libs/interprocess/build//boost_interprocess ;

install conan-install-headers
    : [ glob-tree-ex libs/interprocess/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/interprocess/include
    ;
alias conan-install : libs/interprocess/build//conan-install conan-install-headers ;
