...
class BoostFusionRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'fusion')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/fusion/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_FUSION_NO_LIB=1']

        if len(targets) < 2:
            self.cpp_info.set_property('cmake_target_name', 'Boost::fusion')
            self.cpp_info.set_property('b2_target_name', '/boost/fusion//boost_fusion')
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/fusion//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/fusion//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_fusion : : : <include>include ;
...
alias boost_fusion : libs/fusion/build//boost_fusion ;

install conan-install-headers
    : [ glob-tree-ex libs/fusion/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/fusion/include
    ;
alias conan-install : libs/fusion/build//conan-install conan-install-headers ;
