...
class BoostContextRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'context')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/context/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_CONTEXT_NO_LIB=1']
        ...
        if len(targets) < 2:
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/context//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/context//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_context : : : <include>include ;
...
alias boost_context : libs/context/build//boost_context ;

install conan-install-headers
    : [ glob-tree-ex libs/context/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/context/include
    ;
alias conan-install : libs/context/build//conan-install conan-install-headers ;
