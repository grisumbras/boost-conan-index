...
class BoostRationalRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'rational')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/rational/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_RATIONAL_NO_LIB=1']

        if len(targets) < 2:
            self.cpp_info.set_property('cmake_target_name', 'Boost::rational')
            self.cpp_info.set_property('b2_target_name', '/boost/rational//boost_rational')
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/rational//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/rational//boost_' + name)
                if tgt['kind'] == 'library':
...
project /boost/rational ;

alias boost_rational : : : <include>include ;

call-if : boost-library rational ;
'''
...
alias boost_rational : libs/rational/build//boost_rational ;

install conan-install-headers
    : [ glob-tree-ex libs/rational/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/rational/include
    ;
alias conan-install : libs/rational/build//conan-install conan-install-headers ;
