...
class BoostMysqlRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'mysql')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/mysql/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_MYSQL_NO_LIB=1']

        if len(targets) < 2:
            self.cpp_info.set_property('cmake_target_name', 'Boost::mysql')
            self.cpp_info.set_property('b2_target_name', '/boost/mysql//boost_mysql')
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/mysql//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/mysql//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_mysql : : : <include>include ;
...
alias boost_mysql : libs/mysql/build//boost_mysql ;

install conan-install-headers
    : [ glob-tree-ex libs/mysql/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/mysql/include
    ;
alias conan-install : libs/mysql/build//conan-install conan-install-headers ;
