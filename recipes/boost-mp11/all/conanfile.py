...
class BoostMp11Recipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'mp11')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/mp11/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_MP11_NO_LIB=1']

        if len(targets) < 2:
            self.cpp_info.set_property('cmake_target_name', 'Boost::mp11')
            self.cpp_info.set_property('b2_target_name', '/boost/mp11//boost_mp11')
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/mp11//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/mp11//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_mp11 : : : <include>include ;
...
alias boost_mp11 : libs/mp11/build//boost_mp11 ;

install conan-install-headers
    : [ glob-tree-ex libs/mp11/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/mp11/include
    ;
alias conan-install : libs/mp11/build//conan-install conan-install-headers ;
