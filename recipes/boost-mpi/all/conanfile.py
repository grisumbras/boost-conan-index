...
class BoostMpiRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'mpi')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/mpi/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_MPI_NO_LIB=1']

        if len(targets) < 2:
            self.cpp_info.set_property('cmake_target_name', 'Boost::mpi')
            self.cpp_info.set_property('b2_target_name', '/boost/mpi//boost_mpi')
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/mpi//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/mpi//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_mpi : : : <include>include ;
...
alias boost_mpi : libs/mpi/build//boost_mpi ;

install conan-install-headers
    : [ glob-tree-ex libs/mpi/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/mpi/include
    ;
alias conan-install : libs/mpi/build//conan-install conan-install-headers ;
