...
class BoostIostreamsRecipe(ConanFile):
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/iostreams/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_IOSTREAMS_NO_LIB=1']
        ...
        if len(targets) < 2:
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/iostreams//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/iostreams//boost_' + name)
                if tgt['kind'] == 'library':
