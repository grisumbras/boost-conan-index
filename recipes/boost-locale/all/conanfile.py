...
class BoostLocaleRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'locale')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/locale/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_LOCALE_NO_LIB=1']
        ...
        if len(targets) < 2:
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/locale//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/locale//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_locale : : : <include>include ;
...
alias boost_locale : libs/locale/build//boost_locale ;

install conan-install-headers
    : [ glob-tree-ex libs/locale/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/locale/include
    ;
alias conan-install : libs/locale/build//conan-install conan-install-headers ;
