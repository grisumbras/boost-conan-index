...
class BoostThrowExceptionRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'throw_exception')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def build(self):
        ...
        b2.build('boost_throw_exception')
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/throw_exception/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_THROW_EXCEPTION_NO_LIB=1']

        if len(targets) < 2:
            self.cpp_info.set_property('cmake_target_name', 'Boost::throw_exception')
            self.cpp_info.set_property('b2_target_name', '/boost/throw_exception//boost_throw_exception')
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/throw_exception//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/throw_exception//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_throw_exception : : : <include>include ;
...
alias boost_throw_exception : libs/throw_exception/build//boost_throw_exception ;

install conan-install-headers
    : [ glob-tree-ex libs/throw_exception/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/throw_exception/include
    ;
alias conan-install : libs/throw_exception/build//conan-install conan-install-headers ;
