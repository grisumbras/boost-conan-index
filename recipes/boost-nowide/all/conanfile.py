...
class BoostNowideRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'nowide')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/nowide/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_NOWIDE_NO_LIB=1']
        ...
        if len(targets) < 2:
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/nowide//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/nowide//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_nowide : : : <include>include ;
...
alias boost_nowide : libs/nowide/build//boost_nowide ;

install conan-install-headers
    : [ glob-tree-ex libs/nowide/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/nowide/include
    ;
alias conan-install : libs/nowide/build//conan-install conan-install-headers ;
