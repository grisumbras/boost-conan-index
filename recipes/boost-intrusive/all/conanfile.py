...
class BoostIntrusiveRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'intrusive')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/intrusive/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_INTRUSIVE_NO_LIB=1']

        if len(targets) < 2:
            self.cpp_info.set_property('cmake_target_name', 'Boost::intrusive')
            self.cpp_info.set_property('b2_target_name', '/boost/intrusive//boost_intrusive')
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/intrusive//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/intrusive//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_intrusive : : : <include>include ;
...
alias boost_intrusive : libs/intrusive/build//boost_intrusive ;

install conan-install-headers
    : [ glob-tree-ex libs/intrusive/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/intrusive/include
    ;
alias conan-install : libs/intrusive/build//conan-install conan-install-headers ;
