...
class BoostSortRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'sort')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/sort/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_SORT_NO_LIB=1']

        if len(targets) < 2:
            self.cpp_info.set_property('cmake_target_name', 'Boost::sort')
            self.cpp_info.set_property('b2_target_name', '/boost/sort//boost_sort')
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/sort//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/sort//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_sort : : : <include>include ;

call-if : boost-library sort ;
'''
...
alias boost_sort : libs/sort/build//boost_sort ;

install conan-install-headers
    : [ glob-tree-ex libs/sort/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/sort/include
    ;
alias conan-install : libs/sort/build//conan-install conan-install-headers ;
