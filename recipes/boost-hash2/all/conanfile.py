...
class BoostHash2Recipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'hash2')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/hash2/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_HASH2_NO_LIB=1']

        if len(targets) < 2:
            self.cpp_info.set_property('cmake_target_name', 'Boost::hash2')
            self.cpp_info.set_property('b2_target_name', '/boost/hash2//boost_hash2')
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/hash2//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/hash2//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_hash2 : : : <include>include ;
...
alias boost_hash2 : libs/hash2/build//boost_hash2 ;

install conan-install-headers
    : [ glob-tree-ex libs/hash2/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/hash2/include
    ;
alias conan-install : libs/hash2/build//conan-install conan-install-headers ;
