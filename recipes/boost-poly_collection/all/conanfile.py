...
class BoostPolyCollectionRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'poly_collection')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def build(self):
        ...
        b2.build('boost_poly_collection')
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/poly_collection/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_POLY_COLLECTION_NO_LIB=1']
        ...
        if len(targets) < 2:
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/poly_collection//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/poly_collection//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_poly_collection : : : <include>include ;
...
alias boost_poly_collection : libs/poly_collection/build//boost_poly_collection ;

install conan-install-headers
    : [ glob-tree-ex libs/poly_collection/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/poly_collection/include
    ;
alias conan-install : libs/poly_collection/build//conan-install conan-install-headers ;
