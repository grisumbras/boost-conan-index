...
class BoostWaveRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'wave')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/wave/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_WAVE_NO_LIB=1']

        if len(targets) < 2:
            self.cpp_info.set_property('cmake_target_name', 'Boost::wave')
            self.cpp_info.set_property('b2_target_name', '/boost/wave//boost_wave')
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/wave//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/wave//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_wave : : : <include>include ;
...
alias boost_wave : libs/wave/build//boost_wave ;

install conan-install-headers
    : [ glob-tree-ex libs/wave/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/wave/include
    ;
alias conan-install : libs/wave/build//conan-install conan-install-headers ;
