...
class BoostLockfreeRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'lockfree')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/lockfree/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_LOCKFREE_NO_LIB=1']

        if len(targets) < 2:
            self.cpp_info.set_property('cmake_target_name', 'Boost::lockfree')
            self.cpp_info.set_property('b2_target_name', '/boost/lockfree//boost_lockfree')
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/lockfree//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/lockfree//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_lockfree : : : <include>include ;
...
alias boost_lockfree : libs/lockfree/build//boost_lockfree ;

install conan-install-headers
    : [ glob-tree-ex libs/lockfree/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/lockfree/include
    ;
alias conan-install : libs/lockfree/build//conan-install conan-install-headers ;
