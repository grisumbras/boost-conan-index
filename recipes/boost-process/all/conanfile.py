...
class BoostProcessRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'process')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/process/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_PROCESS_NO_LIB=1']

        if len(targets) < 2:
            self.cpp_info.set_property('cmake_target_name', 'Boost::process')
            self.cpp_info.set_property('b2_target_name', '/boost/process//boost_process')
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/process//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/process//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_process : : : <include>include ;
...
alias boost_process : libs/process/build//boost_process ;

install conan-install-headers
    : [ glob-tree-ex libs/process/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/process/include
    ;
alias conan-install : libs/process/build//conan-install conan-install-headers ;
