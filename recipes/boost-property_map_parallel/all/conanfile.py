...
class BoostPropertyMapParallelRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'property_map_parallel')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def build(self):
        ...
        b2.build('boost_property_map_parallel')
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/property_map_parallel/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_PROPERTY_MAP_PARALLEL_NO_LIB=1']

        if len(targets) < 2:
            self.cpp_info.set_property('cmake_target_name', 'Boost::property_map_parallel')
            self.cpp_info.set_property('b2_target_name', '/boost/property_map_parallel//boost_property_map_parallel')
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/property_map_parallel//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/property_map_parallel//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_property_map_parallel : : : <include>include ;
...
alias boost_property_map_parallel : libs/property_map_parallel/build//boost_property_map_parallel ;

install conan-install-headers
    : [ glob-tree-ex libs/property_map_parallel/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/property_map_parallel/include
    ;
alias conan-install : libs/property_map_parallel/build//conan-install conan-install-headers ;
