...
class BoostAlgorithmRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'algorithm')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/algorithm/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_ALGORITHM_NO_LIB=1']

        if len(targets) < 2:
            self.cpp_info.set_property('cmake_target_name', 'Boost::algorithm')
            self.cpp_info.set_property('b2_target_name', '/boost/algorithm//boost_algorithm')
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/algorithm//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/algorithm//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_algorithm : : : <include>include ;
...
alias boost_algorithm : libs/algorithm/build//boost_algorithm ;

install conan-install-headers
    : [ glob-tree-ex libs/algorithm/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/algorithm/include
    ;
alias conan-install : libs/algorithm/build//conan-install conan-install-headers ;
