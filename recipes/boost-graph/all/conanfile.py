...
class BoostGraphRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'graph')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/graph/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_GRAPH_NO_LIB=1']
        ...
        if len(targets) < 2:
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/graph//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/graph//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_graph : : : <include>include ;
...
alias boost_graph : libs/graph/build//boost_graph ;

install conan-install-headers
    : [ glob-tree-ex libs/graph/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/graph/include
    ;
alias conan-install : libs/graph/build//conan-install conan-install-headers ;
