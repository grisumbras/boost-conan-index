...
class BoostVariant2Recipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'variant2')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/variant2/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_VARIANT2_NO_LIB=1']

        if len(targets) < 2:
            self.cpp_info.set_property('cmake_target_name', 'Boost::variant2')
            self.cpp_info.set_property('b2_target_name', '/boost/variant2//boost_variant2')
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/variant2//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/variant2//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_variant2 : : : <include>include ;
...
alias boost_variant2 : libs/variant2/build//boost_variant2 ;

install conan-install-headers
    : [ glob-tree-ex libs/variant2/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/variant2/include
    ;
alias conan-install : libs/variant2/build//conan-install conan-install-headers ;
