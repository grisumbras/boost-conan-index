...
class BoostRangeRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'range')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/range/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_RANGE_NO_LIB=1']

        if len(targets) < 2:
            self.cpp_info.set_property('cmake_target_name', 'Boost::range')
            self.cpp_info.set_property('b2_target_name', '/boost/range//boost_range')
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/range//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/range//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_range : : : <include>include ;
...
alias boost_range : libs/range/build//boost_range ;

install conan-install-headers
    : [ glob-tree-ex libs/range/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/range/include
    ;
alias conan-install : libs/range/build//conan-install conan-install-headers ;
