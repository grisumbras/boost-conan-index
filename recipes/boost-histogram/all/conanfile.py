...
class BoostHistogramRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'histogram')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/histogram/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_HISTOGRAM_NO_LIB=1']

        if len(targets) < 2:
            self.cpp_info.set_property('cmake_target_name', 'Boost::histogram')
            self.cpp_info.set_property('b2_target_name', '/boost/histogram//boost_histogram')
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/histogram//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/histogram//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_histogram : : : <include>include ;
...
alias boost_histogram : libs/histogram/build//boost_histogram ;

install conan-install-headers
    : [ glob-tree-ex libs/histogram/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/histogram/include
    ;
alias conan-install : libs/histogram/build//conan-install conan-install-headers ;
