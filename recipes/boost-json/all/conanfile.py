...
class BoostJsonRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'json')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/json/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_JSON_NO_LIB=1']

        if len(targets) < 2:
            self.cpp_info.set_property('cmake_target_name', 'Boost::json')
            self.cpp_info.set_property('b2_target_name', '/boost/json//boost_json')
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/json//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/json//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_json : : : <include>include ;
...
alias boost_json : libs/json/build//boost_json ;

install conan-install-headers
    : [ glob-tree-ex libs/json/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/json/include
    ;
alias conan-install : libs/json/build//conan-install conan-install-headers ;
