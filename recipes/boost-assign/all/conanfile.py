...
class BoostAssignRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'assign')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/assign/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_ASSIGN_NO_LIB=1']

        if len(targets) < 2:
            self.cpp_info.set_property('cmake_target_name', 'Boost::assign')
            self.cpp_info.set_property('b2_target_name', '/boost/assign//boost_assign')
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/assign//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/assign//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_assign : : : <include>include ;
...
alias boost_assign : libs/assign/build//boost_assign ;

install conan-install-headers
    : [ glob-tree-ex libs/assign/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/assign/include
    ;
alias conan-install : libs/assign/build//conan-install conan-install-headers ;
