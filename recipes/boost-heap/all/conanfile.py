...
class BoostHeapRecipe(ConanFile):
    name = 'boost-heap'
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'heap')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/heap/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_HEAP_NO_LIB=1']

        if len(targets) < 2:
            self.cpp_info.set_property('cmake_target_name', 'Boost::heap')
            self.cpp_info.set_property('b2_target_name', '/boost/heap//boost_heap')
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/heap//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/heap//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_heap : : : <include>include ;
...
alias boost_heap : libs/heap/build//boost_heap ;

install conan-install-headers
    : [ glob-tree-ex libs/heap/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/heap/include
    ;
alias conan-install : libs/heap/build//conan-install conan-install-headers ;
