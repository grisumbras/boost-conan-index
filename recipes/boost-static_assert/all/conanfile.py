...
class BoostStaticAssertRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'static_assert')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/static_assert/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_STATIC_ASSERT_NO_LIB=1']
        ...
        if len(targets) < 2:
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/static_assert//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/static_assert//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_static_assert : : : <include>include ;
...
alias boost_static_assert : libs/static_assert/build//boost_static_assert ;

install conan-install-headers
    : [ glob-tree-ex libs/static_assert/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/static_assert/include
    ;
alias conan-install : libs/static_assert/build//conan-install conan-install-headers ;
