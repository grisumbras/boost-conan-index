...
class BoostConceptCheckRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'concept_check')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/concept_check/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_CONCEPT_CHECK_NO_LIB=1']

        if len(targets) < 2:
            self.cpp_info.set_property('cmake_target_name', 'Boost::concept_check')
            self.cpp_info.set_property('b2_target_name', '/boost/concept_check//boost_concept_check')
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/concept_check//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/concept_check//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_concept_check : : : <include>include ;
...
alias boost_concept_check : libs/concept_check/build//boost_concept_check ;

install conan-install-headers
    : [ glob-tree-ex libs/concept_check/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/concept_check/include
    ;
alias conan-install : libs/concept_check/build//conan-install conan-install-headers ;
