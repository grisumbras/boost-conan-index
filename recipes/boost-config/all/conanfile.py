...
class BoostConfigRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'config')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/config/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_CONFIG_NO_LIB=1']

        if len(targets) < 2:
            self.cpp_info.set_property('cmake_target_name', 'Boost::config')
            self.cpp_info.set_property('b2_target_name', '/boost/config//boost_config')
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/config//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/config//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_config : : : <include>include ;
...
alias boost_config : libs/config/build//boost_config ;

install conan-install-headers
    : [ glob-tree-ex libs/config/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/config/include
    ;
alias conan-install : libs/config/build//conan-install conan-install-headers ;
