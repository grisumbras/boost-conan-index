...
class BoostCobaltRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'cobalt')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        no_autolink = ['BOOST_COBALT_NO_LIB=1']
        ...
        if len(targets) < 2:
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/cobalt//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/cobalt//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_cobalt : libs/cobalt/build//boost_cobalt ;

install conan-install-headers
    : [ glob-tree-ex libs/cobalt/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/cobalt/include
    ;
alias conan-install : libs/cobalt/build//conan-install conan-install-headers ;
