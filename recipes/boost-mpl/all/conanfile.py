...
class BoostMplRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'mpl')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/mpl/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_MPL_NO_LIB=1']

        if len(targets) < 2:
            self.cpp_info.set_property('cmake_target_name', 'Boost::mpl')
            self.cpp_info.set_property('b2_target_name', '/boost/mpl//boost_mpl')
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/mpl//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/mpl//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_mpl : : : <include>include ;
...
alias boost_mpl : libs/mpl/build//boost_mpl ;

install conan-install-headers
    : [ glob-tree-ex libs/mpl/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/mpl/include
    ;
alias conan-install : libs/mpl/build//conan-install conan-install-headers ;
