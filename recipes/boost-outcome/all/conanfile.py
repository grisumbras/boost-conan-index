...
class BoostOutcomeRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'outcome')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/outcome/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_OUTCOME_NO_LIB=1']

        if len(targets) < 2:
            self.cpp_info.set_property('cmake_target_name', 'Boost::outcome')
            self.cpp_info.set_property('b2_target_name', '/boost/outcome//boost_outcome')
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/outcome//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/outcome//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_outcome : : : <include>include ;
...
alias boost_outcome : libs/outcome/build//boost_outcome ;

install conan-install-headers
    : [ glob-tree-ex libs/outcome/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/outcome/include
    ;
alias conan-install : libs/outcome/build//conan-install conan-install-headers ;
