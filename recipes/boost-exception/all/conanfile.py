...
class BoostExceptionRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'exception')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/exception/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_EXCEPTION_NO_LIB=1']

        if len(targets) < 2:
            self.cpp_info.set_property('cmake_target_name', 'Boost::exception')
            self.cpp_info.set_property('b2_target_name', '/boost/exception//boost_exception')
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/exception//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/exception//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_exception : : : <include>include ;
...
alias boost_exception : libs/exception/build//boost_exception ;

install conan-install-headers
    : [ glob-tree-ex libs/exception/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/exception/include
    ;
alias conan-install : libs/exception/build//conan-install conan-install-headers ;
