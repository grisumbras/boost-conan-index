...
class BoostGeometryRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'geometry')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/geometry/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_GEOMETRY_NO_LIB=1']

        if len(targets) < 2:
            self.cpp_info.set_property('cmake_target_name', 'Boost::geometry')
            self.cpp_info.set_property('b2_target_name', '/boost/geometry//boost_geometry')
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/geometry//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/geometry//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_geometry : : : <include>include ;
...
alias boost_geometry : libs/geometry/build//boost_geometry ;

install conan-install-headers
    : [ glob-tree-ex libs/geometry/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/geometry/include
    ;
alias conan-install : libs/geometry/build//conan-install conan-install-headers ;
