...
class BoostUuidRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'uuid')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/uuid/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_UUID_NO_LIB=1']

        if len(targets) < 2:
            self.cpp_info.set_property('cmake_target_name', 'Boost::uuid')
            self.cpp_info.set_property('b2_target_name', '/boost/uuid//boost_uuid')
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/uuid//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/uuid//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_uuid : : : <include>include ;
...
alias boost_uuid : libs/uuid/build//boost_uuid ;

install conan-install-headers
    : [ glob-tree-ex libs/uuid/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/uuid/include
    ;
alias conan-install : libs/uuid/build//conan-install conan-install-headers ;
