...
class BoostMsmRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'msm')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        no_autolink = ['BOOST_MSM_NO_LIB=1']
        ...
        if len(targets) < 2:
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/msm//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/msm//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_msm : libs/msm/build//boost_msm ;

install conan-install-headers
    : [ glob-tree-ex libs/msm/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/msm/include
    ;
alias conan-install : libs/msm/build//conan-install conan-install-headers ;
