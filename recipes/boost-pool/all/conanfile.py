...
class BoostPoolRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'pool')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        no_autolink = ['BOOST_POOL_NO_LIB=1']
        ...
        if len(targets) < 2:
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/pool//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/pool//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_pool : libs/pool/build//boost_pool ;

install conan-install-headers
    : [ glob-tree-ex libs/pool/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/pool/include
    ;
alias conan-install : libs/pool/build//conan-install conan-install-headers ;
