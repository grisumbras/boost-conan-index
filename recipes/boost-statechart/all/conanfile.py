...
class BoostStatechartRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'statechart')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/statechart/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_STATECHART_NO_LIB=1']
        ...
        if len(targets) < 2:
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/statechart//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/statechart//boost_' + name)
                if tgt['kind'] == 'library':
...
project /boost/statechart ;

alias boost_statechart : : : <include>include ;

call-if : boost-library statechart ;
'''
...
alias boost_statechart : libs/statechart/build//boost_statechart ;

install conan-install-headers
    : [ glob-tree-ex libs/statechart/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/statechart/include
    ;
alias conan-install : libs/statechart/build//conan-install conan-install-headers ;
