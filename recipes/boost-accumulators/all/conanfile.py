...
class BoostAccumulatorsRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'accumulators')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/accumulators/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_ACCUMULATORS_NO_LIB=1']

        if len(targets) < 2:
            self.cpp_info.set_property('cmake_target_name', 'Boost::accumulators')
            self.cpp_info.set_property('b2_target_name', '/boost/accumulators//boost_accumulators')
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/accumulators//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/accumulators//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_accumulators : : : <include>include ;
...
alias boost_accumulators : libs/accumulators/build//boost_accumulators ;

install conan-install-headers
    : [ glob-tree-ex libs/accumulators/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/accumulators/include
    ;
alias conan-install : libs/accumulators/build//conan-install conan-install-headers ;
