...
class BoostRedisRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'redis')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/redis/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_REDIS_NO_LIB=1']

        if len(targets) < 2:
            self.cpp_info.set_property('cmake_target_name', 'Boost::redis')
            self.cpp_info.set_property('b2_target_name', '/boost/redis//boost_redis')
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/redis//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/redis//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_redis : : : <include>include ;
...
alias boost_redis : libs/redis/build//boost_redis ;

install conan-install-headers
    : [ glob-tree-ex libs/redis/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/redis/include
    ;
alias conan-install : libs/redis/build//conan-install conan-install-headers ;
