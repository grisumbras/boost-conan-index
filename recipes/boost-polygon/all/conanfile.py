...
class BoostPolygonRecipe(ConanFile):
    name = 'boost-polygon'
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'polygon')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/polygon/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_POLYGON_NO_LIB=1']

        if len(targets) < 2:
            self.cpp_info.set_property('cmake_target_name', 'Boost::polygon')
            self.cpp_info.set_property('b2_target_name', '/boost/polygon//boost_polygon')
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/polygon//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/polygon//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_polygon : : : <include>include ;

call-if : boost-library polygon ;
'''
...
alias boost_polygon : libs/polygon/build//boost_polygon ;

install conan-install-headers
    : [ glob-tree-ex libs/polygon/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/polygon/include
    ;
alias conan-install : libs/polygon/build//conan-install conan-install-headers ;
