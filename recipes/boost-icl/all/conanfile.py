...
class BoostIclRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'icl')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/icl/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_ICL_NO_LIB=1']

        if len(targets) < 2:
            self.cpp_info.set_property('cmake_target_name', 'Boost::icl')
            self.cpp_info.set_property('b2_target_name', '/boost/icl//boost_icl')
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/icl//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/icl//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_icl : : : <include>include ;
...
alias boost_icl : libs/icl/build//boost_icl ;

install conan-install-headers
    : [ glob-tree-ex libs/icl/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/icl/include
    ;
alias conan-install : libs/icl/build//conan-install conan-install-headers ;
