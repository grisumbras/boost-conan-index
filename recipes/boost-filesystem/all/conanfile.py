...
class BoostFilesystemRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'filesystem')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/filesystem/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_FILESYSTEM_NO_LIB=1']

        if len(targets) < 2:
            self.cpp_info.set_property('cmake_target_name', 'Boost::filesystem')
            self.cpp_info.set_property('b2_target_name', '/boost/filesystem//boost_filesystem')
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/filesystem//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/filesystem//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_filesystem : : : <include>include ;
...
alias boost_filesystem : libs/filesystem/build//boost_filesystem ;

install conan-install-headers
    : [ glob-tree-ex libs/filesystem/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/filesystem/include
    ;
alias conan-install : libs/filesystem/build//conan-install conan-install-headers ;
