...
class BoostPreprocessorRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'preprocessor')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/preprocessor/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_PREPROCESSOR_NO_LIB=1']

        if len(targets) < 2:
            self.cpp_info.set_property('cmake_target_name', 'Boost::preprocessor')
            self.cpp_info.set_property('b2_target_name', '/boost/preprocessor//boost_preprocessor')
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/preprocessor//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/preprocessor//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_preprocessor : : : <include>include ;
...
alias boost_preprocessor : libs/preprocessor/build//boost_preprocessor ;

install conan-install-headers
    : [ glob-tree-ex libs/preprocessor/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/preprocessor/include
    ;
alias conan-install : libs/preprocessor/build//conan-install conan-install-headers ;
