...
class BoostTimerRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'timer')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/timer/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_TIMER_NO_LIB=1']

        if len(targets) < 2:
            self.cpp_info.set_property('cmake_target_name', 'Boost::timer')
            self.cpp_info.set_property('b2_target_name', '/boost/timer//boost_timer')
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/timer//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/timer//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_timer : : : <include>include ;
...
alias boost_timer : libs/timer/build//boost_timer ;

install conan-install-headers
    : [ glob-tree-ex libs/timer/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/timer/include
    ;
alias conan-install : libs/timer/build//conan-install conan-install-headers ;
