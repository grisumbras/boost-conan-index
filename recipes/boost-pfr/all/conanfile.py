...
class BoostPfrRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'pfr')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/pfr/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_PFR_NO_LIB=1']

        if len(targets) < 2:
            self.cpp_info.set_property('cmake_target_name', 'Boost::pfr')
            self.cpp_info.set_property('b2_target_name', '/boost/pfr//boost_pfr')
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/pfr//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/pfr//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_pfr : : : <include>include ;
...
alias boost_pfr : libs/pfr/build//boost_pfr ;

install conan-install-headers
    : [ glob-tree-ex libs/pfr/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/pfr/include
    ;
alias conan-install : libs/pfr/build//conan-install conan-install-headers ;
