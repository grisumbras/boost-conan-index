...
class BoostLeafRecipe(ConanFile):
    name = 'boost-leaf'
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'leaf')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/leaf/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_LEAF_NO_LIB=1']

        if len(targets) < 2:
            self.cpp_info.set_property('cmake_target_name', 'Boost::leaf')
            self.cpp_info.set_property('b2_target_name', '/boost/leaf//boost_leaf')
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/leaf//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/leaf//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_leaf : libs/leaf/build//boost_leaf ;

install conan-install-headers
    : [ glob-tree-ex libs/leaf/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/leaf/include
    ;
alias conan-install : libs/leaf/build//conan-install conan-install-headers ;
