...
class BoostCharconvRecipe(ConanFile):
    name = 'boost-charconv'
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'charconv')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/charconv/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_CHARCONV_NO_LIB=1']

        if len(targets) < 2:
            self.cpp_info.set_property('cmake_target_name', 'Boost::charconv')
            self.cpp_info.set_property('b2_target_name', '/boost/charconv//boost_charconv')
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/charconv//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/charconv//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_charconv : : : <include>include ;
...
alias boost_charconv : libs/charconv/build//boost_charconv ;

install conan-install-headers
    : [ glob-tree-ex libs/charconv/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/charconv/include
    ;
alias conan-install : libs/charconv/build//conan-install conan-install-headers ;
