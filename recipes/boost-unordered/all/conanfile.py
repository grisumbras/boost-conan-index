...
class BoostUnorderedRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'unordered')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/unordered/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_UNORDERED_NO_LIB=1']
        ...
        if len(targets) < 2:
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/unordered//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/unordered//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_unordered : : : <include>include ;
...
alias boost_unordered : libs/unordered/build//boost_unordered ;

install conan-install-headers
    : [ glob-tree-ex libs/unordered/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/unordered/include
    ;
alias conan-install : libs/unordered/build//conan-install conan-install-headers ;
