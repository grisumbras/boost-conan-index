...
class BoostDynamicBitsetRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'dynamic_bitset')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/dynamic_bitset/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_DYNAMIC_BITSET_NO_LIB=1']
        ...
        if len(targets) < 2:
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/dynamic_bitset//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/dynamic_bitset//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_dynamic_bitset : : : <include>include ;
...
alias boost_dynamic_bitset : libs/dynamic_bitset/build//boost_dynamic_bitset ;

install conan-install-headers
    : [ glob-tree-ex libs/dynamic_bitset/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/dynamic_bitset/include
    ;
alias conan-install : libs/dynamic_bitset/build//conan-install conan-install-headers ;
