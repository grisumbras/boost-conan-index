...
class BoostFlyweightRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'flyweight')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/flyweight/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_FLYWEIGHT_NO_LIB=1']

        if len(targets) < 2:
            self.cpp_info.set_property('cmake_target_name', 'Boost::flyweight')
            self.cpp_info.set_property('b2_target_name', '/boost/flyweight//boost_flyweight')
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/flyweight//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/flyweight//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_flyweight : : : <include>include ;
...
alias boost_flyweight : libs/flyweight/build//boost_flyweight ;

install conan-install-headers
    : [ glob-tree-ex libs/flyweight/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/flyweight/include
    ;
alias conan-install : libs/flyweight/build//conan-install conan-install-headers ;
