...
class BoostComputeRecipe(ConanFile):
    ...
    def source(self):
        ...
        if not glob.glob(build_jam):
            if self._is_header_only:
                ...
                files = glob.glob('*')
                lib_dir = os.path.join(self.source_folder, 'libs', 'compute')
                mkdir(self, lib_dir)
                for file in files:
    ...
    def package_info(self):
        ...
        self.cpp_info.builddirs = ['share/boost/compute/modules']

        targets = self._data_cache['targets']
        no_autolink = ['BOOST_COMPUTE_NO_LIB=1']

        if len(targets) < 2:
            self.cpp_info.set_property('cmake_target_name', 'Boost::compute')
            self.cpp_info.set_property('b2_target_name', '/boost/compute//boost_compute')
            ...
            self.cpp_info.set_property('b2_target_name', '/boost/compute//libs')
            for tgt in targets:
                name = tgt['name']
                comp = self.cpp_info.components[name]
                comp.bindirs = []
                comp.set_property('cmake_target_name', 'Boost::' + name)
                comp.set_property('b2_target_name', '/boost/compute//boost_' + name)
                if tgt['kind'] == 'library':
...
alias boost_compute : : : <include>include ;
...
alias boost_compute : libs/compute/build//boost_compute ;

install conan-install-headers
    : [ glob-tree-ex libs/compute/include : *.* ]
    : <location>(includedir)
      <install-source-root>libs/compute/include
    ;
alias conan-install : libs/compute/build//conan-install conan-install-headers ;
